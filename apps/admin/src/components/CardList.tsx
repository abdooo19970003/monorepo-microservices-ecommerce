...
const CardList = ({
  ...
  dataPromise: Promise<cardItemType[]>
  dataType: 'Order' | 'Product'
}) => {
  var items: cardItemType[] = use(dataPromise)
  return (
    <div className='bg-primary-foreground p-4 rounded-lg'>
      <h1 className='text-lg mb-6 font-medium'>{`Latest ${dataType}s`}</h1>
      <div className='flex flex-col gap-2 '>
        {items.map((item) => (
          <Card
            key={item.id}
            className='flex-row justify-between gap-4 p-3 items-center hover:bg-accent-foreground/20 '
          >
            ...
            <CardFooter className='p-0 font-bold'>{item.count} $</CardFooter>
          </Card>
        ))}
      </div>
    </div>
